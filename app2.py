        ...
        if st.button("🧹 Nettoyer et Normaliser", type="primary"):
            with st.spinner("Nettoyage en cours..."):
                ...
                with col1:
                    ...
                    st.dataframe(features_df.describe().T.head(10))
                
                with col2:
                    st.markdown("**Après Normalisation**")
                    st.dataframe(cleaned_features.describe().T.head(10))
...
# ============================================================================
    ...
    if 'cleaning_done' not in st.session_state or not st.session_state['cleaning_done']:
        ...
        st.markdown("### 🎯 Choisissez votre Modèle")
            ...
            if st.button("🚀 Lancer la Prédiction", type="primary"):
                with st.spinner("Prédiction en cours..."):
                    ...
                    if use_rf:
                        # Simulation RF
                        np.random.seed(42)
                        proba_rf = np.random.uniform(0.65, 0.95)
                        pred_rf = 1 if proba_rf > 0.5 else 0
                        predictions['rf'] = {
                            'prediction': pred_rf,
                            'probability': proba_rf,
                            'confidence': proba_rf if pred_rf == 1 else (1 - proba_rf)
                        }
                    
                    if use_xgb:
                        # Simulation XGBoost
                        np.random.seed(43)
                        proba_xgb = np.random.uniform(0.70, 0.98)
                        pred_xgb = 1 if proba_xgb > 0.5 else 0
                        predictions['xgb'] = {
                            'prediction': pred_xgb,
                            'probability': proba_xgb,
                            'confidence': proba_xgb if pred_xgb == 1 else (1 - proba_xgb)
                        }
                    ...
                    for idx, (model_name, result) in enumerate(predictions.items()):
                        with cols[idx]:
                            model_label = "🌲 Random Forest" if model_name == 'rf' else "⚡ XGBoost"
                    ...
                    if len(predictions) == 2:
                        ...
                        pred_rf = predictions['rf']['prediction']
                        pred_xgb = predictions['xgb']['prediction']
                        
                        if pred_rf == pred_xgb:
                            if pred_rf == 1:
                                st.success("✅ **Les deux modèles confirment : Valve OPTIMALE**")
                            else:
                                st.error("❌ **Les deux modèles confirment : Valve NON-OPTIMALE**")
                        else:
                            st.warning("⚠️ **Désaccord entre les modèles - Inspection manuelle recommandée**")
...
# ============================================================================
    ...
    if 'predictions_done' not in st.session_state or not st.session_state['predictions_done']:
        ...
        st.markdown("### 📊 Métriques de Performance (Jeu de Test)")
        
        st.info("""
        Les métriques ci-dessous ont été calculées sur un ensemble de test de **441 cycles** 
        lors de l'entraînement des modèles.
        """)
        
        # Métriques pour chaque modèle utilisé
        cols = st.columns(len(predictions))
        
        # MÉTRIQUES RÉELLES (à ajuster selon tes vrais résultats)
        metrics_data = {
            'rf': {
                'Accuracy': 0.9659,
                'F2-Score': 0.9730,
        ...
        st.markdown("---")
        st.markdown("### 📊 Comparaison Visuelle des Performances")
        
        if len(predictions) == 2:
            comparison_df = pd.DataFrame({
                'Métrique': ['Accuracy', 'F2-Score', 'Recall', 'Precision', 'ROC-AUC'],
                'Random Forest': [metrics_data['rf'][m] for m in ['Accuracy', 'F2-Score', 'Recall', 'Precision', 'ROC-AUC']],
                'XGBoost': [metrics_data['xgb'][m] for m in ['Accuracy', 'F2-Score', 'Recall', 'Precision', 'ROC-AUC']]
            })
            ...
            fig.add_trace(go.Bar(
                ...
                text=[f"{v:.2%}" for v in comparison_df['Random Forest']],
                textposition='auto',
                marker_color='#2ecc71'
            ))
            ...
            fig.add_trace(go.Bar(
                ...
                text=[f"{v:.2%}" for v in comparison_df['XGBoost']],
                textposition='auto',
                marker_color='#3498db'
            ))
        ...
        if len(predictions) == 2:
            ...
            if pred_rf == 1 and pred_xgb == 1:
                st.success("""
                ✅ **Valve en parfait état de fonctionnement**
                ...
                **Actions URGENTES recommandées :**
                ...
                st.warning("""
                ⚠️ **Résultats non-concordants**
                
                **Actions recommandées :**
                - 🔍 Effectuer une inspection visuelle
                - 📊 Collecter des données sur plusieurs cycles
                - 🧪 Lancer un test de diagnostic complet
            ...
            if single_pred['prediction'] == 1:
                st.success("✅ Valve optimale - Aucune action requise")
            else:
                st.error("❌ Valve non-optimale - Inspection recommandée")
